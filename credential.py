...
def check_credentials(credential_file, retry_string, sp, window):
    if not sp:  # only get credentials if credentials doesn't already exist
        ...
        if not credentials_entered:
            ...
            description_label = tk.Label(credentials_window, width=35, text='Please Enter Your Credentials:',
                                         anchor='w')
            client_id_label = tk.Label(credentials_window, width=18, text='Spotify API Client ID', anchor='e')
            client_id_field = tk.Entry(credentials_window, width=20)
            client_secret_label = tk.Label(credentials_window, width=18, text='Spotify API Client Secret', anchor='e')
            ...
            button_submit = ttk.Button(credentials_window, text='Submit', width=10,
                                       command=lambda: valid_credentials(credentials, credential_file,
                                                                         credentials_window,
